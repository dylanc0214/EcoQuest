...
<body>
    ...
    if (!$is_db_connected) {
        ...
        try {
            ...
            // --- 4. FETCH RECENT APPROVED ACTIVITY (Assumes 'activities' table) ---
            $sql_activity = "SELECT quest_name, points_earned, created_at FROM activities WHERE user_id = ? AND status = 'approved' ORDER BY created_at DESC LIMIT 3";
            if ($stmt_activity = $conn->prepare($sql_activity)) {
                ...
                while ($activity = $result_activity->fetch_assoc()) {
                    ...
                    // Since we're here, we can also count completed quests (optional, but useful)
                    $user_metrics['quests_completed']++; 
                }
                $stmt_activity->close();
            }
    ...
    if ($is_db_connected && isset($conn)) {
        // $conn->close(); // Optional: let PHP handle cleanup, but good practice to know it's here
    }
